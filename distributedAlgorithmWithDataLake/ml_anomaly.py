...
# USE_GPU = False
# EPOCHS = 30              # tuned: 15 under-trains the row-wise AE/VAE/USAD/Transformer on this signal
#                           # (empirically ~doubles precision at equal recall on DCS telemetry, see tuning notes)
# BATCH_SIZE = 256
# HIDDEN = 16
# LATENT = 2
# LSTM_WINDOW = 5          # tuned: the DCS faults are shorter and sharper than the network-intrusion
#                           # sequences this was built for; window=10 smeared A3/A4 detection, window=5 sharpens it
...
#     return float(np.percentile(np.sort(dists[:, -1]), 97))  # tuned from 95th
...
#     k = 3 * len(feature_cols)  # tuned from 2x
...
#     # ------------------------------------------------------------------
#     # One-time ingest (run once, outside this pipeline, before this script runs):
#     #   spark.read.option("header", True).option("inferSchema", True) \
#     #       .csv("hdfs://namenode:9000/user/demo/dcs_test_new.csv") \
#     #       .withColumn("ID", F.monotonically_increasing_id()) \
#     #       .write.mode("overwrite").parquet("hdfs://namenode:9000/user/demo/network_data.parquet")
...
#     working_df = threshold_flag(working_df, "kmeans_dist", "kmeans_flag", n_std=2.0)  # tuned from 0.7
 
#     print("\n[MLlib] Algorithm 2: Gaussian Mixture Model...")
#     gmm = GaussianMixture(featuresCol="scaled_features", predictionCol="gmm_cluster",
#                            probabilityCol="gmm_probability", k=6, seed=RANDOM_SEED)  # tuned from k=3
...
#     pca_kmeans = KMeans(featuresCol="pca_features", predictionCol="pca_cluster", k=4, seed=RANDOM_SEED)  # tuned from k=2
...
#     working_df = threshold_flag(working_df, "iso_score", "iso_flag", n_std=2.6)  # tuned from 2.3
...
#     deepemb_kmeans = KMeans(featuresCol="deepemb_vec", predictionCol="deepemb_cluster", k=8, seed=RANDOM_SEED)  # tuned from k=4
...
#                                  probabilityCol="dagmm_probability", k=5, seed=RANDOM_SEED)  # tuned from k=3
...
#     )
#     working_df = threshold_flag(working_df, "dagmm_energy", "dagmm_flag", n_std=3.0)  # tuned from 1.0
...
#     working_df = threshold_flag(working_df, "vae_score", "vae_flag", n_std=1.3)  # tuned from 1.8
...
#     working_df = threshold_flag(working_df, "gan_score", "gan_flag", n_std=2.5)  # tuned from 1.8
...
#     working_df = threshold_flag(working_df, "usad_score", "usad_flag", n_std=1.2)  # tuned from 0.5
...
#     working_df = threshold_flag(working_df, "transformer_score", "transformer_flag", n_std=0.2)  # tuned from 0.75
...
#     lstm_thr = lstm_stats["m"] + 0.5 * lstm_stats["s"]  # tuned from 0.6
...
#     omni_thr = omni_stats["m"] + 0.6 * omni_stats["s"]  # tuned from 0.4 (real cluster run over-flagged at 0.4)
...
#     chosen_threshold, stats_row = rl_tune_adaptive_threshold(df, FEATURE_COLS, target_rate=0.02)  # tuned from 0.05
#     z_conditions = []
#     for c in FEATURE_COLS:
#         mean_c, std_c = stats_row[f"{c}_mean"], max(stats_row[f"{c}_std"], 1e-10)
...
#     min_votes = math.ceil(n_algorithms / 4)  # tuned from n_algorithms / 2 (majority)
...
# ========================================================================
def main():
    spark = SparkSession.builder.appName("Distributed_Anomaly_Ensemble_FullyDistributed") \
    ...
    #       .csv("hdfs://namenode:9000/user/demo/dcs_test_new.csv") \
    #       .withColumn("ID", F.monotonically_increasing_id()) \
    #       .write.format("delta").mode("overwrite").save("hdfs://namenode:9000/user/demo/network_data")
    hdfs_path = "hdfs://namenode:9000/user/demo/network_data"
    df = spark.read.format("delta").load(hdfs_path).select("ID", *FEATURE_COLS)
    id_type = df.schema["ID"].dataType
    total_rows = df.count()
    print(f"[Driver] Loaded {total_rows} rows from HDFS.")
